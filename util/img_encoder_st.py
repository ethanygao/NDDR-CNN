...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255, task='seg'):
    """
    ...
    if task == 'seg':
        ...
        label_crop.set_shape((crop_h, crop_w, 3))
    else:
        raise NotImplementedError
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, random_crop, ignore_label, img_mean,
                          task='seg'):  # optional pre-processing arguments
    ...
    if task == 'seg':
        ...
        label = tf.image.decode_image(label_contents, channels=3)
    ...
    if input_size is not None:
        ...
        if random_crop:
            img, label = random_crop_and_pad_image_and_labels(img, label, h, w, ignore_label, task)
        else:
            img.set_shape((h, w, 3))
            if task == 'seg':
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, data_list, input_size,
                 ...
                 ignore_label, img_mean, coord, task='seg'):
        ...
        self.image, self.label = read_images_from_disk(self.queue, self.input_size, random_scale, random_mirror,
                                                       random_crop, ignore_label, img_mean, task)
